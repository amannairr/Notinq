...
extension NSAttributedString.Key {
    static let aiBlockID = NSAttributedString.Key("ProjectLumoraAIBlockID")
    static let aiBlockRole = NSAttributedString.Key("ProjectLumoraAIBlockRole")
    static let aiBlockActionTitle = NSAttributedString.Key("ProjectLumoraAIBlockActionTitle")
}
...
class TextViewBridge {
    ...
    func appendAIStreamingToken(_ token: String) {
        ...
        let nextLength = range.length + token.utf16.count
    ...
    func startLiveTranscription(modelPath: String = "models/ggml-base.en.bin") -> Bool {
        ...
        guard let whisperManager = WhisperManager(modelPath: resolvedModelPath, threads: 8) else {
            insertStatusMessage(
                textView: textView,
    ...
    private func resolveModelPath(_ modelPath: String) -> String? {
        ...
        let candidates: [String] = [
            ...
            "\(fm.currentDirectoryPath)/ProjectLumora/\(modelPath)",
            "\(fm.currentDirectoryPath)/ggml-base.en.bin",
            "\(fm.currentDirectoryPath)/Models/ggml-base.en.bin",
            "\(fm.currentDirectoryPath)/models/ggml-base.en.bin",
            "\(NSHomeDirectory())/Documents/ProjectLumora/\(modelPath)",
            "\(NSHomeDirectory())/Documents/ProjectLumora/Models/ggml-base.en.bin",
            "\(NSHomeDirectory())/Documents/ProjectLumora/models/ggml-base.en.bin",
            "\(NSHomeDirectory())/ProjectLumora/\(modelPath)"
        ]
